...
init()
{
	...
	// perks - dunno why i dont just mod these here instead of in zom.gsc
	...
	for ( index = 0; index < commandTokens.size; index++ )
	{
		...
		switch (subTokens[0])
		{
			...
			case "loadout_perk3":
			{
				...
				&& getSubStr(subTokens[0], 8) == tablelookup("mp/statsTable.csv", 4, subTokens[1], 8))
				//modwarfare bug o: so need to check loadout_<perkno> matches the perkno in statstable
				//or you can put any stat in any slot using openscriptmenu z loadout_perk1:anyperk
				{
					self.pers[self.class][subTokens[0]] = subTokens[1];
					self setClientDvar( subTokens[0], subTokens[1] );
				}
...
menuAcceptClass()
{
	...
	//note: clear their stowed wep due to a game crashing bug in 1.25-...thanks coverop!
  if (isdefined(self.tag_stowed_back))
    self detach(self.tag_stowed_back, "tag_stowed_back");
    
	if (isdefined(self.tag_stowed_hip))
    self detach(self.tag_stowed_hip, "tag_stowed_hip_rear");
	...
	if ( self.sessionstate == "playing" )
	{
		...
		//if zoms aren't active or they are and in grace period
		if(!level.zombiesactive || (level.zombiesactive && level.zomgrace))
