...
giveLoadout( team, class )
{
	...
	if(primaryWeapon != "none")
	{
		...
		if(self.pers["team"] != "axis")
		{
			...
			if((issubstr(primaryWeapon, "uzi")
			 || issubstr(primaryWeapon, "dragunov")
			 || issubstr(primaryWeapon, "m1014")
			 || issubstr(primaryWeapon, "m60e4")
			 || issubstr(primaryWeapon, "ak47"))
			&& isdefined(rank) && rank >= 99)
				goldweapon = true;

			if(goldweapon)
			{
				self giveWeapon(primaryWeapon, 6);
			}
			else
			{
				//give rand camo including blue camo
				self giveWeapon(primaryWeapon, RandomIntRange(0, 6));
			}
		}
	...
	switch(self.pers[class]["loadout_perk1"])
	{
		...
		case "rpg_mp":
			if(self.pers["team"] != "axis")
			{
				...
				if(level._rpg) self setActionSlot(3, "weapon", "rpg_mp");
			}
			self setActionSlot( 4, "" );
		break;

		default:
			if(level._gl) self setActionSlot(3, "altMode");
			self setActionSlot(4, "");
		break;
	}
