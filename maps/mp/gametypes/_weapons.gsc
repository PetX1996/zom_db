...
init()
{
	...
	precacheItem("poison_mp");
	precacheItem("electric_mp"); //lightning strike
	precacheItem("ice_mp"); //ice blast
	precacheItem("bite_mp"); //dog bite
	precacheItem("laser_mp"); //lastman's superweapon
	precacheItem("skull_mp"); //zombie skull
	precacheItem("defaultweapon_mp"); //zombie knife
...
{
	if(!level.zombiesactive) return false;
	...
	if(!isPrimaryWeapon(weapon)) return false;		
...
getItemWeaponName()
{
	assert(getsubstr(self.classname, 0, 7) == "weapon_");
	weapname = getsubstr(self.classname, 7);
	return weapname;
}
...
updateStowedWeapon()
{
	...
	//detach_all_weapons(); //why's this commented out...the fix for dog hovering?
